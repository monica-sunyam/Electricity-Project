      ...
      next: (data) => {
        if (data?.res && data?.menu?.sidebar) {
          const customOrder: Record<string, number> = {
            'Stromvergleich.png': 1,
            'Gasvergleich.png': 2,
            'Ladestrom.png': 3,
            'Nachtspeicherofen.png': 4,
            'Warmepumpe.png': 5,
          };

          this.sidebarItems = [...data.menu.sidebar].sort(
            (a, b) =>
              (customOrder[a.originalFileName] || 999) - (customOrder[b.originalFileName] || 999),
          );
        }
